    ...
    <div class="card-body p-3">
        @if ($match)
            ...
            @if ($match->result?->winnerHouse)
                <div class="alert alert-success py-2 mt-3 mb-0">Pemenang: <strong>{{ $match->result->winnerHouse->name }}</strong></div>
            @else
                <form action="{{ route('dart.results.store', $match) }}" method="post" class="row g-2 mt-2">
                    @csrf
                    <div class="col-10">
                        <select name="winner_house_id" class="form-select" required>
                            <option value="">Pilih pemenang 501</option>
                            <option value="{{ $match->homeHouse->id }}">{{ $match->homeHouse->name }}</option>
                            <option value="{{ $match->awayHouse->id }}">{{ $match->awayHouse->name }}</option>
                        </select>
                    </div>
                    <div class="col-2 d-grid"><button class="btn btn-primary" type="submit">✓</button></div>
                </form>
            @endif
        @else
            <div class="text-center text-secondary py-4">Belum dijana</div>
        @endif
    </div>
</div>
